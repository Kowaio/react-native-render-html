...
export default class HTML extends PureComponent {
    ...
    componentDidUpdate(prevProps, prevState) {
        const { html, uri, renderers, forceUpdate } = prevProps;
        ...
        if (html !== this.props.html || uri !== this.props.uri || forceUpdate !== this.props.forceUpdate) {
            // If the source changed, register the new HTML and parse it
            this.registerDOM(this.props);
        }
        if (this.state.dom !== prevState.dom || this.state.forceUpdate !== prevState.forceUpdate) {
    ...
    async registerDOM (props = this.props, cb) {
        ...
        if (html) {
            this.setState({ dom: html, loadingRemoteURL: false, errorLoadingRemoteURL: false, forceUpdate: props.forceUpdate });
        } else if (props.uri) {
            try {
